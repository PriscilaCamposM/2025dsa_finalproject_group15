...
# Introduction  
Soybean is the 5th largest commodity produced in the U.S., 2nd in global production, producing 113.3 million metric tons, which contributes $39.81 billion in exports. However, due to high demand, inadequate management and practices had led to yield reductions and deteriorating soil health. 

According to extensive research, one solution that can address the previous issues is the use of cover crops; intensifying cover crop management has the potential to improve soil health, crop productivity, and conservation benefits within no-till systems but it requires site-specific adaptations. 
...
Cereal rye (CR) was used as cover crop, CR was drilled with a seeding rate of 56 kg ha. There were two CR planting timings (early and late), in 2022 the planting dates were Nov. 2 and Nov. 21, and in 2023, Oct. 5 and Nov. 21. CR was terminated 14-21 days pre planting (DPP) and 1-3 days after plating (DAP), herbicide program is shown in the table below. 
![](../Treatments_herbicides.png)

Soybeans were planted at a 371,287 seed/ha seeding rate, in 2023 soybean was planted in June 1st and June 4th in 2024. Soybean was harvested using the middle two rows in each split plot using a small plot combine, adjusting yield to 13% grain moisture and plot length. Harvest dates were 11/16/2023 and 11/26/2024.
...
Based on previous summary and glimpse we need to wrangle both data set
```{r, warning = FALSE, message=FALSE}
...
```
It looks better, for both data set. 
Let's continue with EDA plots.
...
ggplot(grow_sb_yieldw, aes(x = plantedtiming, 
  ...
  geom_jitter()
```
It looks like soybean yield was overall higher in 2023, but it seems termination timings did not had an effect on soybean yield, but cereal rye planting timings did, in 2023 early planting resulted in a higher yield, and in 2024, late planting resulted in higher yield. Let's check the EDA plots for cover crop biomass. 
...
ggplot(grow_cc_yieldw, aes(x = plantedtiming, 
  ...
  geom_jitter()
```
Based on the plots, 2024 had a higher cover crop biomass, but termination timings did not have an effect on biomass, in the other hand planting timing seems to have an effect on biomass, in both years, it looks like early planting resulted in higher biomass compared to late planting. The increase in biomass for 2024 may explain the decrease in soybean yield, it might have created crop-cover crop competition. 
...
Let's start running our models, but first let's start with soybean yield. 
...
```
Based on our Anova table, there are two significant interactions: planting timing, and planting timing within years.
...
All assumptions are met, let's continue. 
...
ggsave("../output/sb_yield_plantedtiming_year.png",
       ...
       height = 5)
```
2024 resulted in significantly lower yield than in 2023, and planting timing had a significant effect in each year, but it is inconsistent, in 2024 late planting resulted in higher yield, but it was not the same for 2023, however, it looks like late planted overall has a positive effect on soybean yield. 

Let's look at our cover crop biomass analysis.
...
Based on our Anova table only planting timings had a significant effect on cover crop yield. 
...
ggsave("../output/cc_yield_plantedtiming.png",
       ...
       height = 5)
```
Early planting resulted in significantly higher cover crop biomass. 


Based on our EDA plot and models, cereal rye biomass in 2023 was lower than 2024, and for both years early planting resulted in higher biomass, which may explain the reduction in soybean yield in 2024.
One observation we noticed on our plots, was the lower weed density in 2024, since Cereal rye was higher in 2024, it can explain why we had lower weed density; historically cover crop can help with weed suppression by creating ground cover and limiting weed growth.

Cereal rye along with an herbicide program can be effective tool. Cover crop can be critical component to creating IWM approaches for use in soybean in the Georgia.  
